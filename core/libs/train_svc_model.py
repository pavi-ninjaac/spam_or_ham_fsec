...
from core.config.config_parser_file import SVCConfig
...
def train_svc():
    ...
    param: dict = asdict(SVCConfig())
    unique_run_name = get_unique_run_name(model_name="svc")
    ...
    with mlflow.start_run(run_name=unique_run_name):
        ...
        mlflow.sklearn.log_model(model, artifact_path="SVC_model")

    backend_logger.info(f"Model training done for SVC with parameters {param}")
